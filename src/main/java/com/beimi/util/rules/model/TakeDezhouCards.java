...
public class TakeDezhouCards extends TakeCards implements Message , java.io.Serializable{
	...
	public TakeDezhouCards(){}
	
	public TakeDezhouCards(Player player){
	...
	public TakeDezhouCards(Player player , TakeCards last){
		this(player, last, true) ;
	}
	
	public TakeDezhouCards(Player player , TakeCards last , boolean take){
		this.userid = player.getPlayuser() ;
		if(last != null){
	...
	public TakeDezhouCards(Player player , boolean allow , byte[] playCards){
		this.userid = player.getPlayuser() ;
		if(playCards == null){
			this.cards = getAIMostSmall(player, 0) ;
		}else{
	...
	public TakeDezhouCards(Player player , byte[] tipcards){
		this.userid = player.getPlayuser() ;
		this.cards = tipcards ;
		if(this.cards!=null){
			this.cardType =  ActionTaskUtils.identification(cards);
	...
	public byte[] search(Player player , TakeCards lastTakeCards){
		...
		if(lastTakeCards!=null && lastTakeCards.getCardType()!=null){
			retValue = this.getSingle(player.getCards(), types, -1 , 1) ;
		}
		return retValue ;
	}
