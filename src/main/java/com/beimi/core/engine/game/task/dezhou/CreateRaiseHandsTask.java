...
public class CreateRaiseHandsTask extends AbstractTask implements ValueWithExpiryTime  , BeiMiGameTask{
	...
	public void execute(){
		DezhouBoard board = (DezhouBoard) CacheHelper.getBoardCacheBean().getCacheObject(gameRoom.getId(), gameRoom.getOrgi());
		...
		sendEvent("lasthands", new GameBoard(firstPlayer.getPlayuser() , board.getCommunityCards(), board.getRatio()) , gameRoom) ;
