...
cc.Class({
    ...
    begin: function () {
        console.log('[ZhaJinHuaBegin] 点击开始游戏按钮');
        if (!this.ready()) {
    ...
    opendeal: function () {
        console.log('[ZhaJinHuaBegin.opendeal] 点击开始游戏按钮');
        if (!this.ready()) {
            this.alert("网络未连接，请检查服务器是否启动");
            return;
        }
    ...
    seeCards: function () {
        console.log('[ZhaJinHuaBegin] 看牌');
    },

    compareCards: function () {
        console.log('[ZhaJinHuaBegin] 比牌');
    },

    fold: function () {
        console.log('[ZhaJinHuaBegin] 弃牌');
    },

    bet: function (amount) {
        console.log('[ZhaJinHuaBegin] 下注:', amount);
    },
    ...
    playcards: function (game, self, x, card, hide) {
        ...
        pokercard.getComponent("BeiMiCard").hide = hide || false;
    ...
    ensureGameBtnBinding: function() {
        console.log('[ZhaJinHuaBegin] 确保按钮绑定...');
        
        if (!this.gamebtn) {
            this.gamebtn = cc.find('Canvas/global/main/game');
            console.log('[ZhaJinHuaBegin] 自动绑定 gamebtn:', this.gamebtn);
        }
        
        if (this.gamebtn) {
            var self = this;
            this.gamebtn.on(cc.Node.EventType.TOUCH_END, function() {
                console.log('[ZhaJinHuaBegin] TOUCH_END 事件触发（直接监听）');
                self.begin();
            }, this);
            console.log('[ZhaJinHuaBegin] 已添加直接监听');
        }
    },
    
    restoreGlobalInteraction: function() {
        console.log('[ZhaJinHuaBegin] 恢复全局交互...');
        
        var canvas = cc.find('Canvas');
        if (canvas) {
            canvas.active = true;
            canvas.opacity = 255;
            console.log('[ZhaJinHuaBegin] Canvas 已恢复');
        }
        ...
        for (var i = 0; i < maskNames.length; i++) {
            ...
            if (mask) {
                mask.active = false;
                console.log('[ZhaJinHuaBegin] 禁用遮罩:', maskNames[i]);
            }
        }
    }
});